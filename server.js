...
${numbered}` }] }],
  ...
  app.post("/api/broll-search", async (req, res) => {
    try {
      ...
      const items = (data?.videos || []).map((v) => {
        ...
        files.sort((a, b) => Math.abs((a.width || 0) - 720) - Math.abs((b.width || 0) - 720)); // ~720p: buena calidad sin descargas gigantes
        const file = files[0];
        if (!file) return null;
  ...
  app.post("/api/broll-plan", async (req, res) => {
    try {
      ...
      const lines = Array.isArray(req.body?.lines) ? req.body.lines.map((l) => String(l || '')) : [];
